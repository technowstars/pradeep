...
# it will takes tags if any tag has asssingned to iam role
...
# it will take tag if any tag assigned to iam user
...
def cloudtrail_event_parser(event):
    ...
    # Get the assumed IAM role name used to create the new EC2 instance(s)//////not unserstood
    ...
    # Extract the event sourceof the EC2 instance creation  //shubham
    returned_event_fields["user_type1"] = event.get("detail").get("userIdentity").get("type")
    # Extract the event sourceof the EC2 instance creation  //shubham
    returned_event_fields["event_name"] = event.get("detail").get("eventName")
...
def lambda_handler(event, context):
    ...
    if event_fields.get("resource_date"):
        ...
        # shubham
    if event_fields.get("user_type1"):
        resource_tags.append(
            {"Key": "user type1", "Value": event_fields["user_type1"]}
        )
         #bhagwat
    if event_fields.get("event_name"):
        resource_tags.append(
            {"Key": "event name", "Value": event_fields["event_name"]}
        )
